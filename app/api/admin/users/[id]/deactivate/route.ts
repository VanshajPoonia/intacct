...
import { deactivateManagedUser } from "@/lib/supabase/admin-users"
...
export async function POST(_request: Request, context: RouteContext) {
  try {
    ...
    await deactivateManagedUser(actor, id)
    return NextResponse.json({ success: true })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to deactivate the user."
