  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="rounded-sm" disabled={isLoading}>
            {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <BookmarkPlus className="mr-2 h-4 w-4" />}
            {triggerLabel}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Save {moduleLabel} View</DialogTitle>
            <DialogDescription>
              Save the current filters, tab, sort, and visible columns for reuse.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Month-end focus"
              />
            </div>
            <div className="flex items-center gap-2">
              <Checkbox
                id="workspace-view-default"
                checked={isDefault}
                onCheckedChange={value => setIsDefault(Boolean(value))}
              />
              <Label htmlFor="workspace-view-default" className="font-normal">
                Set as default view for {moduleLabel.toLowerCase()}
              </Label>
            </div>
          </div>
          <DialogFooter>
