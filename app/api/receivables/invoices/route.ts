...
import { createSupabaseInvoice, getSupabaseInvoices } from "@/lib/supabase/receivables"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await getSupabaseInvoices(
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Failed to load invoices from Supabase.",
      },
      { status: 500 }
    )
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const invoice = await createSupabaseInvoice(payload)
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Failed to create invoice in Supabase.",
      },
      { status: 500 }
    )
  }
